...
const { ModelRegistry, cosine } = require('../src/heavy.cjs');
let loads = [];
...
before(async () => {
  ...
  client = request(false ? app.getHttpServer() : app);
});
after(async () => {
  if (false) await app.close();
});
...
test('route contract has no accidental documentation or admin endpoints', async () => {
  const instance = false ? app.getHttpAdapter().getInstance() : app;
...
test('unavailable readiness returns 503', async () => {
  ...
  try {
    await request(false ? failed.getHttpServer() : failed)
      .get('/ready')
      .expect(503);
  } finally {
    if (false) await failed.close();
  }
});
